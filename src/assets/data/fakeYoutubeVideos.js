...
for (let i = 0; i < 150; i++) {
  let generateChannelName = avoidFullStop(
    faker.lorem.sentence(faker.datatype.number({ min: 1, max: 3 }))
  );
  ...
  data.push({
    ...
    views: faker.helpers.shuffle([
      ...
      faker.datatype.number({
        min: NUM_SHORTNER_RANGE['M'].min,
        max: NUM_SHORTNER_RANGE['M'].max,
      }),
      faker.datatype.number({
        min: NUM_SHORTNER_RANGE['B'].min,
        max: NUM_SHORTNER_RANGE['B'].max,
      }),
    ])[0],
